...
func (w *ModerationWorker) Work(ctx context.Context, job *river.Job[ModerationArgs]) error {
	...
	if w.deps.Moderation == nil || w.deps.Store == nil {
		slog.Warn("moderation job skipped: missing dependencies", "schematic_id", args.SchematicID)
		return nil
	}
	...
	// If already moderated or blacklisted, skip
	if schem.Moderated || schem.Blacklisted {
		slog.Info("moderation job: schematic already resolved, skipping", "schematic_id", args.SchematicID)
		return nil
	}
